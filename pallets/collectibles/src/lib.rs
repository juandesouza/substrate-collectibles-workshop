...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        // Generates and returns the unique id and color
        fn gen_unique_id() -> ([u8; 16], Color) {
            // Create randomness
            let random = T::CollectionRandomness::random(&b"unique_id"[..]).0;

            // Create randomness payload. Multiple collectibles can 
            //be generated in the same block, retaining uniqueness.
            let unique_payload = (
                random,
        ...
        pub fn mint(
            ...
            CollectibleMap::<T>::insert(collectible.unique.id, collectible);
